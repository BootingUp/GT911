...
static const char *goodix_ts_name = "goodix-gt911";
...
static void gtp_esd_switch(struct i2c_client *, s32);
#endif
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
...
#if GTP_ICS_SLOT_REPORT
	input_mt_init_slots(ts->pen_dev, 5, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED);               // 
#else
	...
	input_set_abs_params(ts->pen_dev, ABS_PRESSURE, 0, 255, 0, 0);
...
#if GTP_ICS_SLOT_REPORT
	input_mt_init_slots(ts->input_dev, 5, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED);     // in case of "out of memory"
#else
...
static s32 gtp_fw_startup(struct i2c_client *client)
{
	u8 opr_buf[4];
	s32 ret = 0;
...
void
 *********************************************************/
static void gtp_esd_switch(struct i2c_client *client, s32 on)
{
	struct goodix_ts_data *ts;
